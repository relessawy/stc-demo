package deduction;

/**
 * This class was automatically generated by the data modeler tool.
 */

public class DeductionRequest implements java.io.Serializable {

	static final long serialVersionUID = 1L;

	@org.kie.api.definition.type.Label("Amount")
	private java.lang.Float amount;

	@org.kie.api.definition.type.Label("Status")
	private java.lang.String requestStatus;

	@org.kie.api.definition.type.Label("Deduction Type")
	private java.lang.String deductionType;

	private java.lang.String donateTo;

	public DeductionRequest() {
	}

	public java.lang.Float getAmount() {
		return this.amount;
	}

	public void setAmount(java.lang.Float amount) {
		this.amount = amount;
	}

	public java.lang.String getRequestStatus() {
		return this.requestStatus;
	}

	public void setRequestStatus(java.lang.String requestStatus) {
		this.requestStatus = requestStatus;
	}

	public java.lang.String getDeductionType() {
		return this.deductionType;
	}

	public void setDeductionType(java.lang.String deductionType) {
		this.deductionType = deductionType;
	}

	public java.lang.String getDonateTo() {
		return this.donateTo;
	}

	public void setDonateTo(java.lang.String donateTo) {
		this.donateTo = donateTo;
	}

	public DeductionRequest(java.lang.Float amount, java.lang.String requestStatus, java.lang.String deductionType,
			java.lang.String donateTo) {
		this.amount = amount;
		this.requestStatus = requestStatus;
		this.deductionType = deductionType;
		this.donateTo = donateTo;
	}

	@Override
	public String toString() {
		return "DeductionRequest [amount=" + amount + ", requestStatus=" + requestStatus + ", deductionType="
				+ deductionType + ", donateTo=" + donateTo + "]";
	}

}